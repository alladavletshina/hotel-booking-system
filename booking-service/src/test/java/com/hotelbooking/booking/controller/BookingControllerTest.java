...
class BookingControllerTest {
    ...
    void cancelBooking_ShouldCancelBookingAndReturnOk() {
        // Given
        Long bookingId = 1L;
        Booking cancelledBooking = createBooking(bookingId, 1L, 101L,
                LocalDate.of(2024, 1, 1), LocalDate.of(2024, 1, 5),
                BookingStatus.CANCELLED, "correlation-123");

        // ИСПРАВЛЕНО: используем thenReturn вместо doNothing
    ...
    void cancelBooking_WithNonExistingId_ShouldPropagateException() {
        // Given
        Long bookingId = 999L;
        // ИСПРАВЛЕНО: используем thenThrow вместо doThrow
